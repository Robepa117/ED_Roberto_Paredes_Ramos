...
int main() {
  do {
    ...
    case 1: // Añadir un dato nuevo
      ...
      cout << "BINEVENIDO, DIGITA ( 1 ) E INGRESA LOS DATOS " << endl;
      cout << endl;
      cin >> n;
      leerpersonal(n, x);
      ordenaxfecha(n, x);
...
void leerpersonal(int n, struct trabajador x[]) {
  for (int i = 0; i < n; i++) {
    ...
    cin >> x[i].fecnac.mes;
    cout << endl;
    cout << "annio nacimiento:";
    cin >> x[i].fecnac.annio;
    cout << endl;
  }
}
...
void listado(int n, struct trabajador x[]) {
  ...
  for (int i = 0; i < n; i++)
    cout << x[i].codigo << "\t" << x[i].apellido << "\t" << x[i].nombre << "\t"
         << x[i].fecnac.dia << "/" << x[i].fecnac.mes << "/"
         << x[i].fecnac.annio << endl;
  cout << endl;
}
